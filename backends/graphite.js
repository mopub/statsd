...
if (config.hostname && (config.append_hostname || false)) {
  ...
  if (config.debug) {
    l.log("Using hostname as suffix: " + stat_suffix)
  }
}
  ...
  if (graphiteHost) {
    try {
      ...
      graphite.on('connect', function() {
        ...
        statString += namespace + '.graphiteStats.' + stat_suffix + ' ' + 'last_exception ' + last_exception + ts_suffix;
        statString += namespace + '.graphiteStats.' + stat_suffix + ' ' + 'last_flush '     + last_flush     + ts_suffix;
        statString += namespace + '.graphiteStats.' + stat_suffix + ' ' + 'flush_time '     + flush_time     + ts_suffix;
        statString += namespace + '.graphiteStats.' + stat_suffix + ' ' + 'flush_length '   + flush_length   + ts_suffix;
...
var flush_stats = function graphite_flush(ts, metrics) {
  ...
  for (key in counters) {
    var namespace = counterNamespace.concat([key, stat_suffix]);
    ...
    if (legacyNamespace === true) {
      ...
      statString += 'stats_counts.' + key + ' ' + stat_suffix + ' ' + value          + ts_suffix;
    } else {
  ...
  for (key in timer_data) {
    var namespace = timerNamespace.concat([key, stat_suffix]);
  ...
  for (key in gauges) {
    var namespace = gaugesNamespace.concat([key, stat_suffix]);
    statString += namespace.join(".") + ' ' + gauges[key] + ts_suffix;
    numStats += 1;
  }

  for (key in sets) {
    var namespace = setsNamespace.concat([key, stat_suffix]);
  ...
  if (legacyNamespace === true) {
    statString += prefixStats + '.numStats ' + stat_suffix + ' ' + numStats + ts_suffix;
    statString += 'stats.' + prefixStats + '.graphiteStats ' + stat_suffix + ' ' + 'calculationtime ' + (Date.now() - starttime) + ts_suffix;
    for (key in statsd_metrics) {
      statString += 'stats.' + prefixStats + '.' + key + ' ' + stat_suffix + ' ' + statsd_metrics[key] + ts_suffix;
    }
  } else {
    statString += namespace.join(".") + '.numStats ' + stat_suffix + ' ' + numStats + ts_suffix;
    statString += namespace.join(".") + '.graphiteStats ' + stat_suffix + ' ' + 'calculationtime ' + (Date.now() - starttime) + ts_suffix;
    for (key in statsd_metrics) {
      var the_key = namespace.concat([key, stat_suffix]);
